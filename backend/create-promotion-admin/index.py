...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    '''
    Создание новой акции. Если акция активна — рассылает уведомление всем подписчикам.
    '''
    ...
    if is_active:
        ...
        if subscribers:
            sent_count = _send_promotion_emails(
                subscribers, title, description, discount, new_price, valid_until, promotion_id
            )
    ...
    return {
        'statusCode': 200,
        'headers': {'Access-Control-Allow-Origin': '*', 'Content-Type': 'application/json'},
        'body': json.dumps({'success': True, 'id': promotion_id, 'emails_sent': sent_count}),
        'isBase64Encoded': False
    }
